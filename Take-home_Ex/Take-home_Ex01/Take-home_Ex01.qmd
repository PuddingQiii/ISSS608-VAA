...
As shown in the output above, the `Age` column is currently stored as a character type (`<chr>`), rather than as an integer (`<int>`).
This prevents numerical operations and proper plotting of age-related trends.
Therefore, it is necessary to convert the `Age` variable to an integer format to enable meaningful quantitative analysis.

But the dataset contains a special age value `"90 and over"` which cannot be directly converted to a numeric format.
To maintain consistency and enable numerical analysis, this value was recoded to `90`, representing the lower bound of the age group.
